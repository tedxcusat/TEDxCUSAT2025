...
export default function MerchCard({ product, index, onSelect }: MerchCardProps) {
    return (
        <motion.div
            ...
            className={`merch-card relative group p-6 border border-white/20 hover:border-[#EB0028] bg-black/50 backdrop-blur-sm flex flex-col cursor-pointer transition-all duration-300 ${!product.inStock ? "opacity-60" : ""
                }`}
        >
            {/* Product Image */}
            ...
            <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full py-3 mt-6 bg-transparent border border-[#EB0028] text-white font-clash font-medium tracking-wide transition-colors duration-300 uppercase hover:bg-[#EB0028]"
            >
